...
function HUDFloatingUnitLabel:init(panel)
	...
	self._health_bar = HUDHealthBar:new(self._panel, 0, 20, 128, 8, nil)
	...
	self._panel:set_h(self._health_bar_pointer:bottom())
end
...
function HUDFloatingUnitLabel:set_unit(unit)
	...
	elseif not self._fading_out then
		...
		self._panel:stop()
		self._panel:animate(function (o)
			wait(alive(self._unit) and self._unit:character_damage()._dead and 0 or 1)
