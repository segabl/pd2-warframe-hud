...
---@field new fun(self, parent_panel, upgrade_data, value, duration):HUDBuffListItem
...
function HUDBuffListItem:init(parent_panel, upgrade_data, value, duration)
	...
	self._name = self._panel:text({
		visible = not upgrade_data.hide_name,
		text = upgrade_data.custom and managers.localization:text(upgrade_data.name_id) or managers.localization:text(upgrade_data.name_id):gsub("%s%(.+%)$", ""):pretty(true),
		align = "center",
		color = upgrade_data.is_debuff and WFHud.settings.colors.debuff or WFHud.settings.colors.buff,
		alpha = 0,
		font_size = WFHud.font_sizes.small * font_scale * hud_scale,
